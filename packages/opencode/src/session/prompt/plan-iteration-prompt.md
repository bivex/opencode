...
3. If the feedback is unclear, use the `ask-user` tool to clarify
4. Use the `sub-agent-plan` tool if substantial restructuring is needed, or make targeted edits directly
5. Save the updated plan file using `str-replace-editor` to modify the existing file (preferred) or `save-file` to overwrite it
...
- ALWAYS use the `ask-user` tool when you need user input - NEVER ask questions inline in your response
